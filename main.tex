...
\TODO{briefly outline/motivate the structure/purpose of the paper, including the constraint that we only focus on microbenchmarks}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{sec:challenges}
\section{Microbenchmarking is hard}

Before we discuss the difficulties of microbenchmarking, we should state the criteria one
should keep in mind when analyzing existing approaches. To emphasize the practical
consequences of these criteria, we present them in terms of the features that an ``ideal''
microbenchmarking framework might provide.
...
performance~\cite{HP5e}. There is a body of literature devoted to identifying these factors,
...
structures (see \ref{fig:meandistributions}), which make them difficult to distinguish from
timing variations due to genuine changes in application peformance.

Many of the factors listed above, particular those triggered via the kernel, are referred
to by the literature as ``OS jitter''. Isolating and eliminating OS jitter is a complex
endeavor, as research progress on achieving system quiescence is highly platform-dependent.
Most techniques for controlling jitter revolve around specialized kernel configurations,
compiler modifications, or VM tuning.
...
\TODO{cite/discuss Kalibera's paper on benchmarking in reasonable time, and similar papers}

\subsection{Estimating statistical signficance is hard}

\TODO{cite/discuss papers that assume normality and/or that all benchmark timings are identically distributed}
