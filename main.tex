...
benchmarking methodology suitable for inclusion in both a continuous integration (CI)
pipeline and in a user's manual workflow. Our goal is to define an experimental protocol and
a statistical test for automatically and reproducibly detecting performance regressions with
respect to a given benchmark suite, quantifying detection confidence using rigorous
...
timing measurements and application performance. We review previous research efforts that
identify sources of these variations, and argue that existing benchmarking methodologies
make statistical assumptions that do not appropriately incorporate this research. We present
a statistical model which, in contrast, provides an explicit formulation of timing
variations, and substantiate our argument using data obtained from a small set of mock Julia
benchmarks.
...
Recall that $k$ is the number of instructions in $P_0$. In the extreme case where all
$p^{(i)[j]} = 1$, all $X_P^{(i)} = k$ such that $t_P = t_{P_0} + k \sum_{i} \tau^{(i)}$. If
we assume that the number of delay factors is fixed, then the sum over $i$ is a constant,
and the upper bound on $t_P$ scales linearly with $k$.

In the opposite extreme case, all $p^{(i)[j]} = 0$, all $X_P^{(i)} = 0$ such that $t_P =
t_{P_0}$. Thus, the lower bound on $t_P$ is simply $t_{P_0}$.

It is possible that neither of these extreme behaviors ever occur in reality, but among
the trigger count configurations encountered in a given experiment, there must be at least
one configuration which maximizes $t_P$ to some value $[t_P]_{\textrm{max}} \le t_{P_0} +
k \sum_{i} \tau^{(i)}$, and at least one configuration which minimizes $t_P$ to some value
$[t_P]_{\textrm{min}} \ge t_{P_0}$.

Rephrasing these properties in the language of statisitics, there must exist some critical
time $[t_P]_{\textrm{max}}$ for which the probability density function (pdf) $f_{t_P}(\xi)$
of $t_P$ obeys $f_{t_P}(\xi) = 0 \; \forall \; \xi > [t_P]_{\textrm{max}}$, and some
critical time $[t_P]_{\textrm{min}}$ for which $f_{t_P}(\xi) = 0 \; \forall \; \xi <
[t_P]_{\textrm{min}}$.
...
experiment. In this section, we show that
...
Q_0 &= \left[P_0, P_0, \dots P_0 \right] \\ \nonumber
    &= \left[I_{P}^{[1]}, \dots I_{P}^{[k]}, I_{P}^{[1]}, \dots I_{P}^{[k]}, \dots I_{P}^{[1]}, \dots I_{P}^{[k]} \right] \\ \nonumber
    &= \left[I_{Q}^{[1]}, I_{Q}^{[2]}, \dots I_{Q}^{[nk]} \right]
\end{align}

such that instruction $I_{P}^{[i]} = I_{Q}^{[i + ck]}$ for $c \in \{0, \dots n\}$.

Following the convention of the previous section, we use $Q$ to denote the program that
results from interleaving $Q_0$ with delay instructions. An observed timing measurement $T$
of $Q$ can be decomposed thusly:

\begin{align}
    T &= t_{Q_0} + \epsilon_{\textrm{acc}} + \sum_{i} X_Q^{(i)} \tau^{(i)} \\ \nonumber
      &= n \cdot t_{P_0} + \epsilon_{\textrm{acc}} + \sum_{i} \sum_{j=1}^{nk} x_Q^{(i)[j]} \tau^{(i)}
\end{align}
...
as $\tau_{\textrm{acc}}$. Since $\tau_{\textrm{acc}}$ is a finite value, the rest of our
analysis will simply assume the worst case, where $\epsilon_{\textrm{acc}} = \tau_{\textrm{acc}}$.

We can obtain a naive estimate $\hat{t}_{P_0}$ for $t_{P_0}$ by dividing $T$ by $n$:

\begin{equation}
    \hat{t}_{P_0} = \frac{T}{n} = t_{P_0} + \frac{\tau_{\textrm{acc}}}{n} + \frac{\sum_{i} \sum_{j=1}^{nk} x_Q^{(i)[j]} \tau^{(i)}}{n}
\end{equation}
...
The fact that the delay factor term does not dissipate at large $n$ leads to an important
and controversial realization: \textbf{One cannot necessarily eliminate the effects of
external variation on a single measurement simply by executing the benchmark a large number
of times.} In our model, whether or not increasing $n$ can render the delay factor term
neglible depends entirely on the trigger count configuration, which, as we discussed in
Section~\ref{sec:intro}, is not easily controlled in practice. The upper bound of the delay
factor term, which we derived in the previous section, potentially dwarfs small $t_{P_0}$ at
the limit of large $n$:

\begin{align}
    \lim_{n\to\infty} \lim_{p^{(i)[j]}\to1} \frac{\sum_{i} \sum_{j=1}^{nk} x_Q^{(i)[j]} \tau^{(i)}}{n} &= \lim_{n\to\infty} \frac{nk \sum_{i} \tau^{(i)}}{n} \\ \nonumber
                                                                                                       &= k \sum_{i} \tau^{(i)}
\end{align}
