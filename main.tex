...
where $\epsilon_{\textrm{acc}}$ is the error due to insufficient timer accuracy. We assume
that $\epsilon_{\textrm{acc}}$ is symmetrically distributed between $-\tau_{\textrm{acc}}$
and $\tau_{\textrm{acc}}$.

We can obtain a naive estimate for $t_{P_0}$ by dividing $T$ by $n$:

\begin{equation}
    \frac{T}{n} = t_{P_0} + \frac{\epsilon_{\textrm{acc}}}{n} + \frac{\sum_{i} \sum_{j=1}^{nk} x_Q^{(i)[j]} \tau^{(i)}}{n}
\end{equation}

In the limit of large $n$, $\frac{\epsilon_{\textrm{acc}}}{n}$ goes to zero, but the
behavior of the delay factor term is less clear:

\begin{equation}
    \lim_{n\to\infty} \frac{T}{n} = t_{P_0} + \lim_{n\to\infty} \frac{\sum_{i} \sum_{j=1}^{nk} x_Q^{(i)[j]} \tau^{(i)}}{n}
\end{equation}

Previously, we derived a tight upper bound on the delay factor term from the worst case
where $p^{(i)[j]} = 1$. By examining this worst case in the limit of large $n$, we see that
the delay factor term can dwarf $t_{P_0}$ regardless of the number of executions of $P_0$
...
Eq~\ref{eq:10} is a key result of our model: \textbf{One cannot necessarily eliminate the
effects of external variation on a single measurement simply by executing the benchmark a
large number of times.} Whether or not increasing $n$ can render the delay factor term
neglible depends entirely on the trigger count configuration, which, as we discussed in
Section~\ref{sec:environment}, is not easily controlled in practice.
...
Given a benchmarkable program $P_0$ and a time budget $\tau_{\textrm{budget}}$, an execution
strategy should maximize the number of timing measurements we make while minimizing the
difference between $t_{P_0}$ and our estimates $\hat{t}_i = \frac{T_i}{n}$. Based on the
first criteria, it is reasonable for our strategy to consist of repeatedly measuring $P_0$'s
runtime until we have exhausted our time budget.

The sole remaining parameter available for optimization then becomes $n$, the number of
program executions that performed per measurement. Our model shows that increasing $n$ can
reduce error due to timer inaccuracy, but does not reliably reduce performance variations
due to external sources. Furthermore, increasing $n$ increases the time required to perform
a single measurement, thus decreasing the number of measurements we are able make within the
time budget. For these reasons, we seek to design an algorithm for determining the
\textit{minimal} value of $n$ necessary to overcome error due to timer inaccuracy. Given a
timer accuracy $\tau_{\textrm{acc}}$ and a timer precision $\tau_{\textrm{prec}}$, our
algorithm for determining the optimal value of $n$ is as follows:

\begin{algorithm}
    \caption{estimating the optimal $n$ value}
    \KwIn{$P_0$, $\tau_{\textrm{acc}}$, $\nu : t_i \to n_i$}
    \KwOut{$n$}
    ...
    Evaluate $\nu(\hat{t}_{P_0})$ to obtain a value of $n$ to use for all subsequent timing measurements of $P_0$.
\end{algorithm}
