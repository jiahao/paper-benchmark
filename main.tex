...
A benchmark execution $t$ is the smallest time scale, referring to how long it takes to execute the benchmark once. We normally do not time single executions.

A timing $T_i$ is the smallest experimentally measured quantity, consisting of multiple executions of the benchmark. The subscript $i$ labels each timing.

A trial is the largest grouping of time, referring to a collection of timings. When we talk about multiple trials, $k_{trials}$ is the number of trials and each trial is indexed by an embraced subscript, $\cdot^{\{i\}}$. The $i$th trial has $k^{\{i\}}$ timings, and $T^{\{i\}}_j$ is the $j$th timing in the $i$th trial.
...
$P_0$, $P$ and $Q$ denote programs that we want to run as benchmarks. $I^{[i]}_{P_0}$ is the $i$th instruction in program $P_0$, where each instruction is indexed by a bracketed superscript, $\cdot^{[i]}$. $D^{[i]}_{P}$ is a delay instruction, which we define precisely in Section~\ref{sec:statmodel}.

$\tau^{(i)}$ is a time scale associated with a noise factor, as defined below. Noise factors are indexed by parenthesized superscripts, $\cdot^{(i)}$.
...
$\epsilon$ is a measurement error and has units of time.
...
$\nu(t)$ is an oracle function where given a time estimate, returns the number of executions per measurement.
