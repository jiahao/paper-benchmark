...
Developers of high performance applications need tests not only for correctness,
but also to detect performance degradation.
However, modern computers and operating systems form an extremely complicated
environment around user programs~\cite{HP5e}.
Interactions with test programs and their environments produce variations in the
observed execution time, which complicates the determination of performance
regressions.

Many external factors conspire to produce variability in timing measurements.
Physical factors like high ambient temperature may trigger changes in power
consumption behavior designed to limit heat production. The system running the
benchmark may also experience many interrupts, e.g.\ many network requests from
a DDoS attack.
...
- implement security features like address space layout randomization, which
  makes the memory access behavior of test programs nondeterministic.

Furthermore, seemingly irrelevant configuration parameters like the size of the
UNIX environment can confound reproducibility. Changing the size of the environment moves the call stack, which is loaded after the environment, which in turn changes the memory alignment of data~\cite{Mytkowicz2009}.
