    ...
    - Introduction:
        - Subsection: Problems:
            ...
            - taking a sufficient number of timing measurements to make reasonable conclusions
              takes a long time
        - Subsection: Existing assumptions/strategies that are wrong/insufficient:
...
methods, our method does not assume the normality of timing measurement distributions, which
we show is often an inappropriate assumption in practice. Additionally, our method respects
experimental time constraints by maximizing the number of measurements made per trial,
rendering it suitable for inclusion in continuous integration (CI) pipelines even when
applied to relatively large benchmark suites.
...
language and ecosystem, which has already caught and prevented several performance
regressions into the core language.
\end{abstract}
...
the impact of code changes on program performance. Despite the importance of these
suites in safeguarding against performance regressions, developers often run and interpret
them in an ad-hoc manner. This lack of a rigorous microbenchmarking methodology is
dangerous, at best wasting developers' time and at worst encouraging misguided code
alterations that damage performance instead of improve it.

Ignoring platform-specific bugs, a proper experimental protocol for microbenchmarking should
allow developers with disparately configured machines to reproducibly arrive at similar
conclusions, even if the timing measurements themselves will be
machine-dependent~\cite{Oliveira2014,Kalibera2013}. Unfortunately, modern hardware and
operating systems present a large number of confounding factors that complicate a
developer's ability to reason about the reproducibility of user-space application
performance~\cite{HP5e}. There is a body of work in the literature devoted to identifying
these factors, examples of which include...
...
\begin{itemize}
    ...
    \item ...the ability of the CPU to pipeline instructions
    \item ...problems with hardware performance counters? \TODO{elaborate/cite this}.
    \item ...insufficient timer accuracy, which can cause measurement bias and
    error~\cite{Mytkowicz2009}.
\end{itemize}

Timing variations due to the above external factors can exhibit perplexing correlation
structures (see \TODO{Fig 1}), which make them difficult to distinguish from timing
variations due to genuine changes in application peformance.

In the following sections, we outline existing approaches, establish the problems these
approaches
...
\subsection{Environmental Approaches}
Many of the external variation factors listed above, particular those triggered via the
kernel, are referred to by the literature as ``OS jitter''. Isolating and eliminating OS
jitter is a complex endeavor, and research progress on achieving system quiescence is highly
OS, hardware and version dependent. Most techniques for controlling jitter revolve around
specialized kernel configurations, compiler modifications, or VM tuning.

Among the compiler-specific techniques are efforts in memory randomization \cite{Curtsinger2013},
....

Most studies we found on VM performance were JVM-specific. ~\cite{Georges2008}

Research on OS-level configuration is unsurprisingly skewed towards Linux-based systems,
with a few papers
% Should we include this? Does this only affect the reproducibility of *data*, or does it
% also affect the reproducibility of *conclusions*?
...
Changing the heap size changes the performance of garbage collections - cite - Blackburn et al. [4], Stephen M Blackburn, Perry Cheng, and Kathryn S McKin- ley. Myths and realities: The performance impact of garbage collection. In SIGMETRICS, pages 25–36. ACM Press, 2004.
...
\caption{Mean distributions for 4 different benchmarks}
